      ...
      <section className="mx-auto max-w-[1380px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10 lg:py-36">
        ...
        <div className="mt-12 grid gap-px overflow-hidden rounded-2xl border border-line bg-line lg:grid-cols-3">
          {[
            ...
            <article key={step} className="bg-card p-7"><div className="flex items-center gap-3"><span className="font-mono text-xs font-black text-brand-2">{step}</span><Icon className="size-5 text-brand" /></div><h2 className="mt-6 text-xl font-black text-fg">{title}</h2><p className="mt-3 text-sm leading-7 text-muted">{body}</p></article>
          ))}
        </div>
      </section>
      ...
      <section className="border-y border-line bg-surface">
        <div className="mx-auto max-w-[1380px] px-4 py-20 sm:px-6 sm:py-28 lg:px-10">
          ...
          <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { cmd: '/sdd "add OAuth switching"', desc: 'Full workflow: plan, implement, verify. Best for well-understood features.' },
              { cmd: '/sdd status', desc: 'Check current phase and progress. Useful when resuming interrupted workflows.' },
              { cmd: '/sdd --skip-plan "task"', desc: 'Skip planning and jump to implementation. Use when you already have a plan.' },
              { cmd: '/sdd --verify-only', desc: 'Run only verification against the current diff. No planning or implementation.' },
