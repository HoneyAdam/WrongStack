...
## Super Memory invariants

`SuperMemoryStore` is the default `TOKENS.MemoryStore` on CLI/TUI/Desktop/standalone WebUI. Canonical revisions live in project-local `.wrongstack/memories/memories.jsonl`; all mutating sequences use the project-wide store lock, readers invalidate cached replay state when another process appends, and JSONL readers share the append lock so partial lines are never quarantined as corruption. Exact/cosmetic duplicates are merged at write time within the same effective scope, preserving tags, anchors, sources, and the strongest quality metadata.

Automatic memory injection is host-parity behavior: CLI and standalone WebUI install tool-call middleware by default; ordinary request/turn injection is opt-in (`superMemory.inject.turnContext: true`). The deterministic Memory Injector folds live todo/Kanban state into the concrete tool query, expands direct seeds through shared file/symbol/package/command/tag metadata and the graph, prioritizes long-lived structural kinds, diversifies the bounded result, and scales its 8-hint/2800-char default against current context pressure. Tool paths resolve from the live working directory before project-relative anchor lookup; write/edit/replace/patch targets re-verify affected anchors (unless `superMemory.hygiene.autoOnFileChange` is false). Only active memory is automatic guidance; stale memory may be surfaced as a warning after mutation verification, while deleted tombstones are never placed in model context. Injection is relevance-gated, size-capped, session-cooldown-aware, fail-open, and suppresses facts already present in the system/tool result. Graph structure connects memory → symbol/file/directory/package/command, symbol → file, file → ancestor directories, and memory → session/tool/source nodes. Keep `core` independent of `super-memory`.
