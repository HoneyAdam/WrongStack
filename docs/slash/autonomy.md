...
| `eternal` | ETERNITY | red | Goal-driven sense/decide/execute/reflect loop. Requires `/goal`. Forces YOLO on, so non-denied tool calls auto-run. Runs until `/autonomy stop`, Ctrl+C twice, or `/goal pause`. |
| `eternal-parallel` | PARALLEL | magenta | Fan-out 4–8 subagents per tick. Each tick: decompose → spawn → await → aggregate → loop. Requires `/goal`. Forces YOLO on, so non-denied tool calls auto-run. |
