# /worktree — Git worktree isolation for Goal

Aliases: `/wt`

## What it does

Goal can run each **phase** in its own git **worktree + branch** so that
...
```
WRONGSTACK_GOAL_WORKTREES=0
```

With isolation off (or outside a git repo), Goal falls back to the legacy
single-tree, one-phase-at-a-time behavior.

## Visual surfaces

- **TUI** — a compact worktree panel sits beside the Goal phase panel
  (branch · owner phase · `+ins/-del` · status). Press **Ctrl+T** for the
  full-screen worktree monitor (Ctrl+T is taken by delete-word).
- **WebUI** — the Goal screen shows a live **swim-lane** band (one lane
...
- [`/goal`](./goal.md) — the run that creates these worktrees.
