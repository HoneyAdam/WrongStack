# @wrongstack/super-memory — Refactor & Bug Report

> **Analysis Date:** 2026-07-21  
> **Scope:** `packages/super-memory/src/` (7,478 lines across 16 modules)  
...
### C1. `SqliteSuperMemoryStore` — No SQL Injection Protection on FTS5 Query

**File:** `packages/super-memory/src/sqlite-store.ts`  
**Lines:** ~640–668  
**Severity:** CRITICAL
...
**File:** `packages/super-memory/src/store.ts`  
**Lines:** 3121–3191  
...
**File:** `packages/super-memory/src/store.ts`  
**Lines:** ~1439–1568 (updateSuperMemory path)  
**Severity:** HIGH
...
### H3. `SqliteSuperMemoryStore` — Missing `contextPolicy` Filter in Queries
...
**File:** `packages/super-memory/src/middleware/injection-tracker.ts`  
**Lines:** 159–179  
**Severity:** HIGH
...
**File:** `packages/super-memory/src/middleware/tool-call-memory.ts`  
**Lines:** 128–129  
**Severity:** HIGH
...
The first parameter is `memory.scope` (of type `SuperMemoryScope`), but `normalizeProjectPath` expects `projectRoot: string`. Since JavaScript coerces any value to string, this silently works but produces incorrect normalization for `session`/`symbol` scopes. The function should receive `this.projectRoot`.
...
The cooldown map is module-level (closure variable in `createSuperMemoryToolCallMiddleware`). `pruneCooldowns()` only fires when `seen.size > 10_000`. In a long-lived session with 100s of memory injections per tool call, this map can grow to 10K+ entries, all kept alive in memory.
...
**File:** `packages/super-memory/src/graph/graph.ts`  
...
**File:** `packages/super-memory/src/middleware/turn-memory.ts`  
**Lines:** 227–258  
**Severity:** MEDIUM
...
**File:** `packages/super-memory/src/middleware/memory-injector-agent.ts`  
**Lines:** 58–68  
**Severity:** LOW
...
**Suggested split:**
```
store.ts → SuperMemoryStore (core CRUD + loadMemories + runMutation)
           ├── SuperMemoryHygieneEngine   (hygiene pipeline)
           ├── SuperMemoryGraphEngine     (auto-edges, proposals)
           ├── SuperMemoryPagination      (listSuperPage, cursor)
           ├── SuperMemoryFileDrawer      (findMemoriesForFile)
           └── SuperMemoryBackfill        (recovery, backfill)
```
...
- Memory: 10K `SuperMemory` objects ≈ 50–100 MB
...
| `SqliteSuperMemoryStore.hygiene` | None | SQLite hygiene path is untested |
...
- Preserves the `SuperMemory` shape
- Maintains backward compatibility with the existing `SuperMemoryStore` constructor
- Does not change the `SqliteSuperMemoryStore` database schema
...
```bash
pnpm --filter @wrongstack/super-memory test
pnpm --filter @wrongstack/super-memory typecheck
```
