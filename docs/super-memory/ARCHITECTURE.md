# @wrongstack/super-memory — Architecture Reference

> **Package:** `@wrongstack/super-memory`  
> **Version:** `0.295.0`  
> **License:** MIT  
> **Author:** ECOSTACK TECHNOLOGY OÜ  
> **Source:** `packages/super-memory/src/` (12 modules, ~7,500 lines)  
> **Tests:** `packages/super-memory/tests/` (27 files, ~4,500+ lines)
...
Super Memory is WrongStack's **project-local, persistent, structured long-term memory** system. It lives at `.wrongstack/memories/` per project, backed by JSONL files (primary) or optionally SQLite (high-performance).
...
│                  SuperMemoryStore                     │
│  ┌─────────────┐  ┌──────────────┐  ┌─────────────┐ │
│  │ JSONL Engine │  │ Graph Engine  │  │ Audit Log   │ │
│  └─────────────┘  └──────────────┘  └─────────────┘ │
│  ┌─────────────┐  ┌──────────────┐                    │
│  │ SQLite Alt  │  │ Anchor Verify│                    │
│  └─────────────┘  └──────────────┘                    │
└──────────────────────────────────────────────────────┘
```
...
### 2.1 `SuperMemory` — Primary Record

```typescript
interface SuperMemory {
  id: string;                    // ULID-based unique ID
  revision: number;              // Incremented on every mutation
  scope: SuperMemoryScope;       // project | user | session | file | symbol
  kind: SuperMemoryKind;         // 13 distinct types
  status: SuperMemoryStatus;     // 6 lifecycle states
...
The types module provides bidirectional converters between Super Memory types and the legacy `@wrongstack/core` `MemoryEntry`/`MemoryScope`/`MemoryType` types:

- `superToLegacyScope()` / `legacyToSuperScope()`
- `kindToLegacyType()` / `legacyTypeToKind()`
- `toLegacyEntry()` — full SuperMemory to MemoryEntry conversion
...
### 4.1 `SuperMemoryStore` (JSONL) — Primary
...
- Each line is a `SuperMemoryRecord` with `recordType`, `schemaVersion`, `op` (create/update/delete), and the full `SuperMemory` object
...
| `updateSuperMemory()` | ✅ | Mutation | Patch a memory, update revision, link graphs |
| `deleteSuperMemory()` | ✅ | Mutation | Soft-delete with cascade (cleans supersedes/contradicts refs in other memories) |
| `recoverSuperMemory()` | ✅ | Mutation | Restore `deleted` to `active` (or return head-of-chain for `superseded`) |
...
### 4.2 `SqliteSuperMemoryStore` (SQLite) — Performance Alternative
...
CREATE TABLE memories (
  id TEXT PRIMARY KEY,
  data TEXT NOT NULL,           -- Full SuperMemory as JSON
...
- `resolveSuperMemoryPaths()`: Validates directory is relative and inside project

---

## 5. Middleware Layer

Super Memory has two middleware pipelines that automatically inject relevant memories into LLM context. This is the most sophisticated part of the system.
...
```
--- Super Memory: related project knowledge ---
