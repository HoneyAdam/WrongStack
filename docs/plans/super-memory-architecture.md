...
Super Memory is the project-local long-term knowledge layer for WrongStack. It is not a bigger
...
- Inject relevant memory automatically into tool results and turn-level context, with strict
  noise controls.
- Keep `core` at the bottom of the dependency graph. Super Memory must depend on `core`, not the
  other way around.
...
```text
core  <-  super-memory  <-  cli/runtime/webui-server/apps
```

`packages/super-memory` may import `@wrongstack/core` types and utilities. `packages/core` must
not import `super-memory`.

Runtime wiring binds:

```ts
TOKENS.MemoryStore -> SuperMemoryStore
```

`SuperMemoryStore` implements the existing `MemoryStore` interface and also exposes richer
methods through a separate `SuperMemoryService` interface.
...
```text
--- Super Memory: notes for packages/core/src/storage ---
...
```ts
export interface SuperMemoryInjectionConfig {
  enabled: boolean;
  toolResults: boolean;
  turnContext: boolean;
...
They delegate to Super Memory through the `MemoryStore` interface. Richer tools live in
`packages/super-memory/src/tools`.
...
export interface SuperMemoryConfig {
  ...
  inject?: SuperMemoryInjectionConfig;
  hygiene?: {
...
1. Add `packages/super-memory` package skeleton and public types.
2. Add project-local JSONL storage, locks, snapshots, and replay.
3. Implement `SuperMemoryStore implements MemoryStore`.
