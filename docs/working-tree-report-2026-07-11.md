...
- **TypeScript 7:** compatibility was explicitly tested. The latest `tsup@8.5.1` DTS bundle crashes against TypeScript 7 compiler internals before project code runs, so the workspace remains pinned to the working 6.0.x line. This is an upstream build-tool constraint, not an unverified upgrade item.
