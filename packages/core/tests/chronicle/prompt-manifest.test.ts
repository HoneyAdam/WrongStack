...
describe('prompt composition manifest', () => {
  it('creates deterministic lineage without retaining prompt or tool prose', () => {
    const request = {
      ...
      messages: [
        ...
        { role: 'assistant' as const, content: [
          ...
          { type: 'image' as const, source: { type: 'base64' as const, media_type: 'image/png', data: 'private-image' } },
        ] },
      ],
      tools: [{ name: 'read', description: 'private tool prose', inputSchema: { type: 'object', properties: { path: { type: 'string' } } }, permission: 'auto' as const, mutating: false, riskTier: 'safe' as const, execute: async () => ({}) }],
      temperature: 0.2,
    };
    ...
    expect(first).toMatchObject({ messageCount: 2, roleCounts: { user: 1, assistant: 1 }, blockCounts: { text: 1, tool_use: 1, image: 1 }, tools: { count: 1, names: ['read'] } });
