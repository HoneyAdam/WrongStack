...
describe('startSddRun (integration — real SddParallelRun + coordinator)', () => {
  ...
  it('drains set_task_model / cancel_task / delete_task control commands into the live run', async () => {
    ...
    await boardStore.appendControl(handle.runId, { ts: 2, type: 'cancel_task', payload: { taskId: t3 } });
    await boardStore.appendControl(handle.runId, { ts: 3, type: 'delete_task', payload: { taskId: t4 } });
