...
describe('validateFleetEventEmission', () => {
  ...
  it('rejects bug.found finding without id', () => {
    const err = validateFleetEventEmission('bug.found', {
      finding: { type: 'security', severity: 'high', location: { file: 'x.ts', line: 5 }, description: 'desc' },
    }, 'bug-hunter');
  ...
  it('rejects bug.found finding without type', () => {
    const err = validateFleetEventEmission('bug.found', {
      finding: { id: 'b1', severity: 'high', location: { file: 'x.ts', line: 5 }, description: 'desc' },
    }, 'bug-hunter');
  ...
  it('rejects bug.found finding with invalid severity', () => {
    const err = validateFleetEventEmission('bug.found', {
      finding: { id: 'b1', type: 'security', severity: 'unknown', location: { file: 'x.ts', line: 5 }, description: 'desc' },
    }, 'bug-hunter');
  ...
  it('rejects bug.found finding without description', () => {
    const err = validateFleetEventEmission('bug.found', {
      finding: { id: 'b1', type: 'security', severity: 'high', location: { file: 'x.ts', line: 5 } },
    }, 'bug-hunter');
  ...
  it('rejects bug.found with non-string suggestedFix', () => {
    const err = validateFleetEventEmission('bug.found', {
      finding: { id: 'b1', type: 'security', severity: 'high', location: { file: 'x.ts', line: 5 }, description: 'desc', suggestedFix: 123 },
    }, 'bug-hunter');
  ...
  it('accepts valid bug.found', () => {
    const err = validateFleetEventEmission('bug.found', {
      finding: { id: 'b1', type: 'security', severity: 'high', location: { file: 'x.ts', line: 5 }, description: 'desc', suggestedFix: 'fix it' },
    }, 'bug-hunter');
  ...
  it('validates refactor.plan plan.id', () => {
    const err = validateFleetEventEmission('refactor.plan', {
      plan: { basedOnBugIds: [], phases: [], riskScore: 'low', estimatedChangeCount: 0, rollbackStrategy: 'rb' },
    }, 'refactor-planner');
  ...
  it('validates refactor.plan basedOnBugIds', () => {
    const err = validateFleetEventEmission('refactor.plan', {
      plan: { id: 'r1', basedOnBugIds: 'not-array', phases: [], riskScore: 'low', estimatedChangeCount: 0, rollbackStrategy: 'rb' },
    }, 'refactor-planner');
  ...
  it('validates refactor.plan phases', () => {
    const err = validateFleetEventEmission('refactor.plan', {
      plan: { id: 'r1', basedOnBugIds: [], phases: [{ number: 'not-number', title: '', tasks: [], risk: 'low' }], riskScore: 'low', estimatedChangeCount: 0, rollbackStrategy: 'rb' },
    }, 'refactor-planner');
  ...
  it('validates refactor.plan riskScore', () => {
    const err = validateFleetEventEmission('refactor.plan', {
      plan: { id: 'r1', basedOnBugIds: [], phases: [], riskScore: 'extreme', estimatedChangeCount: 0, rollbackStrategy: 'rb' },
    }, 'refactor-planner');
  ...
  it('validates refactor.plan estimatedChangeCount', () => {
    const err = validateFleetEventEmission('refactor.plan', {
      plan: { id: 'r1', basedOnBugIds: [], phases: [], riskScore: 'low', estimatedChangeCount: -1, rollbackStrategy: 'rb' },
    }, 'refactor-planner');
  ...
  it('validates refactor.plan rollbackStrategy', () => {
    const err = validateFleetEventEmission('refactor.plan', {
      plan: { id: 'r1', basedOnBugIds: [], phases: [], riskScore: 'low', estimatedChangeCount: 0, rollbackStrategy: '' },
    }, 'refactor-planner');
  ...
  it('accepts valid refactor.plan', () => {
    const err = validateFleetEventEmission('refactor.plan', {
      plan: { id: 'r1', basedOnBugIds: ['b1'], phases: [{ number: 1, title: 'Phase 1', tasks: ['t1'], risk: 'low' }], riskScore: 'low', estimatedChangeCount: 5, rollbackStrategy: 'revert' },
    }, 'refactor-planner');
    expect(err).toBeUndefined();
  });
  ...
  it('validates critic.evaluation evaluation.id', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { subjectType: 'bug_finding', subjectId: 's1', score: 5, verdict: 'approve', strengths: [], weaknesses: [], concerns: [] },
    }, 'critic');
  ...
  it('validates critic.evaluation subjectType', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'unknown', subjectId: 's1', score: 5, verdict: 'approve', strengths: [], weaknesses: [], concerns: [] },
    }, 'critic');
  ...
  it('validates critic.evaluation subjectId', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'bug_finding', subjectId: '', score: 5, verdict: 'approve', strengths: [], weaknesses: [], concerns: [] },
    }, 'critic');
  ...
  it('validates critic.evaluation score range', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'bug_finding', subjectId: 's1', score: 11, verdict: 'approve', strengths: [], weaknesses: [], concerns: [] },
    }, 'critic');
  ...
  it('validates critic.evaluation verdict', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'bug_finding', subjectId: 's1', score: 5, verdict: 'unknown', strengths: [], weaknesses: [], concerns: [] },
    }, 'critic');
  ...
  it('validates critic.evaluation strengths and weaknesses', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'bug_finding', subjectId: 's1', score: 5, verdict: 'approve', strengths: 'not-array', weaknesses: [], concerns: [] },
    }, 'critic');
  ...
  it('validates critic.evaluation concerns', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'bug_finding', subjectId: 's1', score: 5, verdict: 'approve', strengths: [], weaknesses: [], concerns: [{ description: '', severity: 'blocking' }] },
    }, 'critic');
  ...
  it('accepts valid critic.evaluation', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'bug_finding', subjectId: 's1', score: 5, verdict: 'approve', strengths: ['good'], weaknesses: ['bad'], concerns: [{ description: 'concern', severity: 'advisory' }] },
    }, 'critic');
    expect(err).toBeUndefined();
  });
  ...
  it('accepts critic.evaluation with optional location', () => {
    const err = validateFleetEventEmission('critic.evaluation', {
      evaluation: { id: 'e1', subjectType: 'bug_finding', subjectId: 's1', score: 5, verdict: 'approve', strengths: [], weaknesses: [], concerns: [{ description: 'concern', severity: 'blocking', location: { file: 'x.ts', line: 10 } }] },
    }, 'critic');
    expect(err).toBeUndefined();
  });
});
