    ...
    <main className="flex min-w-0 flex-1 flex-col">
      ...
      <section className="border-b border-border/70 px-4 py-3">
        <div className="grid grid-cols-2 gap-2 xl:grid-cols-[1.35fr_1fr_1fr_1fr_1fr_auto]">
          ...
          <div className="flex gap-1.5"><button onClick={apply} className="rounded-md bg-primary px-3 text-xs font-medium text-primary-foreground"><Filter className="mr-1 inline h-3 w-3"/>Apply</button><button onClick={clear} className="rounded-md border border-border px-2.5 text-xs hover:bg-muted">Clear</button></div>
        </div>
      </section>

      <section className="flex items-center gap-1 overflow-x-auto border-b border-border/70 px-4 py-2">
        {signals.map((item) => <button key={item.id} onClick={() => setSignal(item.id)} className={cn('flex shrink-0 items-center gap-1.5 rounded-md px-2.5 py-1.5 text-[11px] transition-colors [&>svg]:h-3.5 [&>svg]:w-3.5', signal === item.id ? 'bg-primary/10 font-medium text-primary' : 'text-muted-foreground hover:bg-muted hover:text-foreground')}>{item.icon}{item.label}<span className="ml-1 rounded bg-background/70 px-1 font-mono text-[9px]">{signalTotal(summary,item.id,meta.total)}</span></button>)}
