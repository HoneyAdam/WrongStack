...
describe('lint-gate extra coverage', () => {
  it('health() returns correct message when lastResult is null and counters are zero', async () => {
    ...
    const h = await lintGatePlugin.health!() as { ok: boolean; message: string; counters: Record<string, number>; lastResult: unknown };
  ...
  it('hook returns undefined for non-write/edit toolName', async () => {
    const api = makeApi();
    lintGatePlugin.setup(api as never);
    const hook = (api.registerHook.mock.calls[0] as unknown[])[2] as (input: Record<string, unknown>) => Promise<unknown>;
  ...
  it('edit hook returns undefined when old_string is missing', async () => {
    ...
    lintGatePlugin.setup(api as never);
    const hook = (api.registerHook.mock.calls[0] as unknown[])[2] as (input: Record<string, unknown>) => Promise<unknown>;
    const result = await hook({ toolName: 'edit', toolInput: { path: 'test.ts', new_string: 'hello' } });
    expect(result).toBeUndefined();
  });
  ...
  it('edit hook returns undefined when new_string is missing', async () => {
    ...
    lintGatePlugin.setup(api as never);
    const hook = (api.registerHook.mock.calls[0] as unknown[])[2] as (input: Record<string, unknown>) => Promise<unknown>;
    const result = await hook({ toolName: 'edit', toolInput: { path: 'test.ts', old_string: 'bye' } });
    expect(result).toBeUndefined();
  });
  ...
  it('teardown handles hook unregister throwing', async () => {
    const api = makeApi();
    api.registerHook = vi.fn(() => { throw new Error('unregister fails'); });
