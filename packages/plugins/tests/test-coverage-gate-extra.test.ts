...
describe('test-coverage-gate extra coverage', () => {
  it('health() returns correct message without lastResult', async () => {
    ...
    const h = await coverageGatePlugin.health!() as { ok: boolean; message: string; counters: Record<string, number>; lastResult: unknown };
  ...
  it('health() returns message with lastResult after a hook run', async () => {
    ...
    writeFileSync(covPath, JSON.stringify({
      total: { lines: { pct: 85 }, statements: { pct: 80 }, functions: { pct: 90 }, branches: { pct: 75 } },
    }));

    const api = makeApi({ 'test-coverage-gate': { coveragePath: covPath, threshold: 0, mode: 'warn' } });
    coverageGatePlugin.setup(api as never);
  ...
  it('hook increments errorCount when coverage file not found', async () => {
    const api = makeApi({ 'test-coverage-gate': { coveragePath: join(projectRoot, '.nonexistent-coverage.json') } });
    coverageGatePlugin.setup(api as never);
    const hook = (api.registerHook.mock.calls[0] as unknown[])[2] as (input: Record<string, unknown>) => unknown;
    await hook({
      toolName: 'write',
  ...
  it('hook returns pass when coverage is above threshold', async () => {
    ...
    writeFileSync(covPath, JSON.stringify({
      total: { lines: { pct: 90 }, statements: { pct: 85 }, functions: { pct: 88 }, branches: { pct: 82 } },
    }));
  ...
  it('block mode returns decision block when coverage below threshold', async () => {
    ...
    writeFileSync(covPath, JSON.stringify({
      total: { lines: { pct: 30 }, statements: { pct: 25 }, functions: { pct: 35 }, branches: { pct: 20 } },
    }));

    const api = makeApi({ 'test-coverage-gate': { coveragePath: covPath, threshold: 80, mode: 'block' } });
    coverageGatePlugin.setup(api as never);
    const hook = (api.registerHook.mock.calls[0] as unknown[])[2] as (input: Record<string, unknown>) => unknown;
    const result = await hook({
      toolName: 'write',
