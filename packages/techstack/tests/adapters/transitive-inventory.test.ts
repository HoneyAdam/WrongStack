...
async function run(ecosystem: EcosystemId, adapter: EcosystemAdapter, files: Record<string, string>, manifest: string) {
  ...
  const workspace: Workspace = { id: `ws-${ecosystem}`, relativeRoot: '.', ecosystem, manifests: [join(root, manifest)], lockfiles: [], confidence: 1, coverage: 'full' };
...
describe('includeTransitive', () => {
  it('adds npm lock-only packages', async () => {
    const deps = await run('npm', npmAdapter, {
      'package.json': JSON.stringify({ dependencies: { direct: '^1.0.0' } }),
      'pnpm-lock.yaml': "lockfileVersion: '9.0'\nimporters:\n  .:\n    dependencies:\n      direct:\n        specifier: ^1.0.0\n        version: 1.1.0\npackages:\n  direct@1.1.0: {}\n  transitive@2.0.0: {}\n",
    }, 'package.json');
    expect(deps.find((dep) => dep.name === 'transitive')).toMatchObject({ direct: false, scope: 'transitive', locked: '2.0.0' });
  });

  it('adds Cargo.lock-only crates', async () => {
    const deps = await run('rust', rustAdapter, {
      'Cargo.toml': '[dependencies]\nserde = "1"\n',
      'Cargo.lock': '[[package]]\nname = "serde"\nversion = "1.0.0"\n[[package]]\nname = "syn"\nversion = "2.0.0"\n',
    }, 'Cargo.toml');
    expect(deps.find((dep) => dep.name === 'syn')).toMatchObject({ direct: false, scope: 'transitive' });
  });

  it('adds poetry.lock-only Python packages', async () => {
    const deps = await run('python', pythonAdapter, {
      'pyproject.toml': '[project]\ndependencies = ["requests>=2"]\n',
      'poetry.lock': '[[package]]\nname = "requests"\nversion = "2.32.0"\n[[package]]\nname = "urllib3"\nversion = "2.2.2"\n',
    }, 'pyproject.toml');
    expect(deps.find((dep) => dep.name === 'urllib3')).toMatchObject({ direct: false, locked: '2.2.2' });
  });

  it('parses a 500-package pnpm lockfile within one second', async () => {
    const packages = Array.from({ length: 500 }, (_, index) => `  package-${index}@1.0.${index}: {}`).join('\n');
