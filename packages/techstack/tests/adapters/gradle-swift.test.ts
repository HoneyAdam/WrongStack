...
function fixture(ecosystem: EcosystemId, files: Record<string, string>, manifests: string[], lockfiles: string[] = []) {
...
describe('GradleAdapter', () => {
  it('parses DSL coordinates, version catalogs, lockfiles, and transitives', async () => {
    const { root, workspace } = fixture('gradle', {
    ...
    const deps = await gradleAdapter.inventory(workspace, { projectRoot: root, includeTransitive: true });
    expect(deps.find((dep) => dep.name === 'org.slf4j:slf4j-api')?.locked).toBe('2.0.13');
    expect(deps.find((dep) => dep.name === 'org.junit.jupiter:junit-jupiter')?.scope).toBe('development');
...
describe('SwiftAdapter', () => {
  it('parses Package.swift, Package.resolved, revisions, and transitives', async () => {
    const { root, workspace } = fixture('swift', {
      'Package.swift': '.package(url: "https://github.com/apple/swift-argument-parser.git", from: "1.2.0")',
      'Package.resolved': JSON.stringify({ version: 2, pins: [
        { identity: 'swift-argument-parser', location: 'https://github.com/apple/swift-argument-parser.git', state: { version: '1.3.0', revision: 'abc' } },
        { identity: 'swift-log', location: 'https://github.com/apple/swift-log.git', state: { revision: 'def' } },
      ] }),
    }, ['Package.swift'], ['Package.resolved']);
    const deps = await swiftAdapter.inventory(workspace, { projectRoot: root, includeTransitive: true });
