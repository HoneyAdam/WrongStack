...
function mkWorkspace(files: Record<string, string>): { dir: string; ws: Workspace } {
  ...
  return { dir, ws: { id: workspaceId('', 'rust'), relativeRoot: dir, ecosystem: 'rust' as const, manifests: Object.keys(files).filter(f => f !== 'Cargo.lock'), lockfiles: Object.keys(files).filter(f => f === 'Cargo.lock'), confidence: 0.9, coverage: 'full' as const } };
}
