...
function stubState(over: Partial<State> = {}): State {
  return {
    ...
    settingsPicker: {
      ...
      streamFleet: true, chime: false, confirmExit: true,
...
describe('reduceFleetState', () => {
  ...
  it('fleetToolStart: returns state unchanged for unknown id', () => {
    const state = stubState();
    expect(reduceFleetState(state, { type: 'fleetToolStart', id: 'unknown', name: 'read' })).toBe(state);
  });
  ...
  it('leaderIterEnd: clears iterating', () => {
    const state = stubState({ leader: { iterations: 5, toolCalls: 3, recentTools: [], currentTool: undefined, startedAt: 0, lastEventAt: 0, iterating: true } });
  ...
  it('leaderCtxPct: clamps and sets context percentage', () => {
    const state = stubState();
    const result = reduceFleetState(state, { type: 'leaderCtxPct', load: 1.5, tokens: 1000, maxContext: 2000 });
    expect((result!).leader.ctxPct).toBe(1);
    expect((result!).leader.ctxTokens).toBe(1000);
  });

  it('setStreamFleet: toggles streamFleet flag', () => {
    const state = stubState();
    const result = reduceFleetState(state, { type: 'setStreamFleet', enabled: false });
    expect((result!).streamFleet).toBe(false);
  });
});
