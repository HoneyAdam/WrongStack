...
describe('reducer — last branches', () => {
  ...
  it('confirmOpen adds to queue, confirmClose shifts', () => {
    const s = reducer(initial(), { type: 'confirmOpen', info: { text: 'Sure?' } });
  ...
  it('collabSubagentSpawned bootstraps collabSession', () => {
    const r = reducer(initial(), { type: 'collabSubagentSpawned', role: 'bug-hunter', goal: 'find bugs' });
  ...
  it('collabSubagentSpawned is no-op when collabSession exists', () => {
    const s = reducer(initial(), { type: 'collabSubagentSpawned', role: 'hunter', goal: 'hunt' });
    const r = reducer(s, { type: 'collabSubagentSpawned', role: 'second', goal: 'more' });
    expect(r.collabSession?.timeline).toHaveLength(1); // not extended
  });
  it('collabBugFound bootstraps with description', () => {
    const r = reducer(initial(), { type: 'collabBugFound', sessionId: 's1', description: 'null pointer', severity: 'high' });
    expect(r.collabSession).not.toBeNull();
    expect(r.collabSession?.bugCount).toBe(1);
  });
  it('collabBugFound appends to existing', () => {
    const s = reducer(initial(), { type: 'collabBugFound', sessionId: 's1', description: 'bug1', severity: 'high' });
    const r = reducer(s, { type: 'collabBugFound', sessionId: 's1', description: 'bug2', severity: 'medium' });
    expect(r.collabSession?.bugCount).toBe(2);
  });
  it('collabSessionDone sets verdict', () => {
    const s = reducer(initial(), { type: 'collabBugFound', sessionId: 's1', description: 'bug', severity: 'low' });
    const r = reducer(s, { type: 'collabSessionDone', sessionId: 's1', verdict: 'approve', score: 9 });
    expect(r.collabSession?.overallVerdict).toBe('approve');
  });
  it('collabSessionDone no-op when no session', () => {
    const r = reducer(initial(), { type: 'collabSessionDone', sessionId: 's1', verdict: 'approve', score: 9 });
