...
/** Known reducer action types — the authoritative allow-list. */
export const ALLOWED_ACTION_TYPES = [
  ...
  'settingsPickerClose',
  'settingsPick',
  'statuslinePickerOpen',
  'statuslinePickerMove',
  'statuslinePickerClose',
  'pluginPickerOpen',
  'pluginPickerMove',
  'pluginPickerClose',
  'mcpPickerOpen',
...
export function validateAction(action: {
  ...
  switch (normalized) {
    ...
    default:
      // Should be unreachable if ALLOWED_ACTION_TYPES is kept in sync above
      return {
        valid: false,
        error: `action.type: "${type}" has no type-specific validator — may be a missing case.`,
      };
  }
}
...
export function safeDispatch(
  ...
  if (!result.valid) {
    ...
    console.error(`[input-validation] REJECTED dispatch "${action.type}": ${result.error}`);
    return false;
  }
