...
export class SuperMemoryGraph {
  ...
  async add(
    ...
    return withFileLock(
      `${this.edgesLog}.mutation`,
      async () => {
        const existing = (await this.list()).find(
          (edge) =>
            !edge.deletedAt && edge.from === from && edge.to === to && edge.relation === relation,
        );
        ...
        const edge: MemoryGraphEdge = {
          ...
          ...(evidence?.length ? { evidence: [...new Set(evidence)].slice(0, 8) } : {}),
  ...
  /** Add a bounded relationship batch with one read and one append. */
  async addMany(
    inputs: Array<{
      from: string;
      to: string;
    ...
    return withFileLock(
      ...
      async () => {
        ...
        const keys = new Set(existing.map((edge) => `${edge.from}\0${edge.to}\0${edge.relation}`));
        const created: MemoryGraphEdge[] = [];
        for (const input of inputs) {
          if (!VALID_RELATIONS.has(input.relation) || input.from === input.to) continue;
          const key = `${input.from}\0${input.to}\0${input.relation}`;
          const reverseKey = `${input.to}\0${input.from}\0${input.relation}`;
          if (keys.has(key) || keys.has(reverseKey)) continue;
          keys.add(key);
          const edge: MemoryGraphEdge = {
            schemaVersion: 1,
            id: `edge_${ulid()}`,
            ...
            ...(input.evidence?.length
              ? { evidence: [...new Set(input.evidence)].slice(0, 8) }
              : {}),
            createdAt: this.now().toISOString(),
          };
          created.push(edge);
        ...
        if (created.length > 0) {
          await appendFile(
            this.edgesLog,
            `${created.map((edge) => JSON.stringify(edge)).join('\n')}\n`,
            'utf8',
          );
        }
        return created;
  ...
  async traverse(
    ...
    while (queue.length > 0 && result.length < limit) {
      ...
      for (const edge of edges) {
        ...
        const next =
          edge.from === current.node ? edge.to : edge.to === current.node ? edge.from : undefined;
