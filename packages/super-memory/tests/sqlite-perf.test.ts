...
describe('SqliteSuperMemoryStore performance paths', () => {
  it('findRelatedSuper returns tag/anchor related memories without scanning the whole corpus for scoring misses', async () => {
    ...
    const relatedByTag = await store.rememberSuper({
      text: 'JWT rotation policy',
      kind: 'convention',
      tags: ['auth'],
      importance: 0.7,
    });
    const relatedByPath = await store.rememberSuper({
    ...
    const related = await store.findRelatedSuper([seed.id], { limit: 20 });
    ...
    console.log(`[sqlite-perf] findRelatedSuper N=${N + 3} → ${related.length} hits in ${ms.toFixed(1)}ms`);
  });
  ...
  it('batch remember + search stays within soft budget', async () => {
    ...
    const results = await store.searchSuper('topic-7', { limit: 15 });
