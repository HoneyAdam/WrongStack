...
import type { RememberSuperMemoryInput } from '../src/types.js';
...
describe('validateRememberInput', () => {
  const minimal: RememberSuperMemoryInput = {
    text: 'hello',
    scope: 'project',
    kind: 'fact',
  };
  ...
  it('rejects invalid scope', () => {
    ...
    ).toThrow('Invalid Super Memory scope');
  });

  it('rejects invalid kind', () => {
    expect(() =>
      validateRememberInput({ ...minimal, kind: 'invalid' as never }),
    ).toThrow('Invalid Super Memory kind');
  });
});
