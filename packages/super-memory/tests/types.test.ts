...
import type { SuperMemory, SuperMemoryKind } from '../src/types.js';

describe('superToLegacyScope', () => {
  it('maps user to user-memory', () => {
    expect(superToLegacyScope('user')).toBe('user-memory');
  });

  it('maps project to project-memory', () => {
    expect(superToLegacyScope('project')).toBe('project-memory');
  });

  it('maps session to project-memory', () => {
    expect(superToLegacyScope('session')).toBe('project-memory');
  });

  it('maps file to project-memory', () => {
    expect(superToLegacyScope('file')).toBe('project-memory');
  });

  it('maps symbol to project-memory', () => {
    expect(superToLegacyScope('symbol')).toBe('project-memory');
  });
});

describe('legacyToSuperScope', () => {
  it('maps user-memory to user', () => {
    expect(legacyToSuperScope('user-memory')).toBe('user');
  });

  it('maps project-agents to project', () => {
    expect(legacyToSuperScope('project-agents')).toBe('project');
  });

  it('maps project-memory to project', () => {
    expect(legacyToSuperScope('project-memory')).toBe('project');
  });
});

describe('kindToLegacyType', () => {
  const testCases: [SuperMemoryKind, string][] = [
    ['decision', 'decision'],
...
describe('toLegacyEntry', () => {
  ...
  it('falls back to superToLegacyScope when legacyScope is undefined', () => {
