...
describe('MCP HTTP fault-injection soak', () => {
  ...
  it(
    'recovers from intermittent HTTP failures and tracks operational health',
    { timeout: 45_000 },
    async () => {
      ...
      const handlers: ((url: string, init: { body?: string }) => Promise<Response> | Response)[] = [
        ...
        (_u, init) => {
          const body = JSON.parse(init.body ?? '{}');
          return jsonRes({ jsonrpc: '2.0', id: body.id, result: { content: 'ok-1', isError: false } });
        },
        ...
        (_u, init) => {
          const body = JSON.parse(init.body ?? '{}');
          return jsonRes({ jsonrpc: '2.0', id: body.id, result: { content: 'ok-2', isError: false } });
        },
        // tools/call #5 success
        (_u, init) => {
          const body = JSON.parse(init.body ?? '{}');
          return jsonRes({ jsonrpc: '2.0', id: body.id, result: { content: 'ok-3', isError: false } });
        },
      ];
  ...
  it(
    'gives up after persistent HTTP transport failures and marks the server failed',
    { timeout: 60_000 },
    async () => {
  ...
  it(
    'handles alternating connect and call failures without leaking connected state',
    { timeout: 45_000 },
    async () => {
