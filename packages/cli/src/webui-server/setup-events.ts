...
export function createSetupEvents(deps: SetupEventsDeps): () => void {
  ...
  return function setupEvents(): void {
    ...
    const projectRoot = deps.agent.ctx.projectRoot || process.cwd();
    ...
    eventUnsubscribers.push(
      deps.events.on('file.activity', (e) => {
        broadcast({ type: 'codemap.file_event', payload: e });
      }),
    );
    ...
    eventUnsubscribers.push(
      ...
      deps.events.on('subagent.tool_started', (e) => {
        broadcast({
