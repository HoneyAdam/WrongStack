 ...
 *
 * EXAMPLES
 *   node scripts/super-memory-maintenance.mjs                 # dry-run report, cwd
 *   node scripts/super-memory-maintenance.mjs --apply         # recover + purge >30d
 *   node scripts/super-memory-maintenance.mjs --purge-days 90 --apply
...
async function loadStoreCtor() {
  ...
  for (const load of candidates) {
    try {
      ...
      if (mod?.SuperMemoryStore) return mod.SuperMemoryStore;
      lastErr = new Error('SuperMemoryStore export missing');
    } catch (err) {
      lastErr = err;
    }
  }
